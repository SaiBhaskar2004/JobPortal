...
{% for job in jobs %}
  <div class="job-card">
    <h3>{{ job.title }}</h3>
    <p>{{ job.description }}</p>
    <p><strong>Category:</strong> {{ job.category }} | <strong>Location:</strong> {{ job.location }} | <strong>Salary:</strong> {{ job.salary }}</p>
...
def index():
    ...
    if request.method == 'POST':
        ...
        if company:
            query = query.filter(Job.posted_by.ilike(f'%{company}%'))
    jobs = query.all()
...
def post_job():
    ...
    if request.method == 'POST':
        ...
        job = Job(title=title, description=description, salary=salary, location=location, category=category, posted_by=session['username'])
        db.session.add(job)
        db.session.commit()
